...
try:
	...
	cursor=con.cursor()
	cursor.execute("select * from ruta WHERE cod='"+str(id)+"'")
	con.commit()
	con.close()
	a=""
	for art in cursor:
	...
	cursor1=con1.cursor()
	cursor1.execute("update ruta set accion="+str(c)+" where cod="+str(id))
	con1.commit()
	con1.close()
	print("<script>location.href='../../administracion/rutas.py'</script>")
except:
