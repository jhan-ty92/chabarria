...
try:
	...
	cursor=con.cursor()
	cursor.execute("select * from frecuencia WHERE cod='"+str(id)+"'")
	con.commit()
	con.close()
	a=""
	for art in cursor:
	...
	cursor1=con1.cursor()
	cursor1.execute("update frecuencia set accion="+str(c)+" where cod="+str(id))
	con1.commit()
	con1.close()
	print("<script>location.href='../../administracion/frecuencia.py'</script>")
except:
